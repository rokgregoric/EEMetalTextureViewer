...
struct PVPlaneDescriptor {
  ...
  static func ==(lhs: PVPlaneDescriptor, rhs: PVPlaneDescriptor) -> Bool {
    lhs.data == rhs.data && lhs.size == rhs.size && lhs.rowBytes == rhs.rowBytes
  }
}
...
class EEPixelBufferViewer: EETextureViewer {
  ...
  var planeDescriptors: [PVPlaneDescriptor] = [] {
    didSet {
      if planeDescriptors.count == 0 { return }

      if sourceImageSize == nil || pixelFormat == nil {
        Log.error("Cannot set planeDescriptors. pixelFormat and sourceImageSize must initialized first", context: "metal")
        return
      }
...
//            Convert here!
        ...
        for (planeIndex, plane) in planeParameters.enumerated() {
          if  textures[planeIndex] != nil && compareTextureObjectWithPlaneDescriptor(texture: textures[planeIndex], planeDescriptor: plane.planeDescriptor) {
  ...
  func compareTextureObjectWithPlaneDescriptor(texture: MTLTexture!, planeDescriptor: PVPlaneDescriptor) -> Bool {
    ...
    if texture.width != Int(planeDescriptor.size.width) || texture.height != Int(planeDescriptor.size.height) { return false }
    ...
    Log.dev(String(format: "texture uses address %x, descriptor has %x %@", calculatedBufferAddress, Int(bitPattern: planeDescriptor.data), calculatedBufferAddress == Int(bitPattern: planeDescriptor.data) ? "" : "MISMATCH"), context: "metal")
    Log.dev(String(format: "address1 %x, address2 %x", calculatedBufferAddress, lastBufferAddress), context: "metal")

    return calculatedBufferAddress == lastBufferAddress
  }
  ...
  fileprivate func makeTextureFromPlaneDescriptor(_ plane: PVInternalPlaneParameters) -> MTLTexture {
    ...
    if texture != nil {
      ...
      if lastBufferUnaligned == false {
        Log.warning("Unaligned buffer -- pixel buffer must be copied for each frame.", context: "metal")
...
//    assert(bytesPerRow % 64 == 0 && offset % 64 == 0, "Supplied bufferPtr and bytesPerRow must be aligned on a 64-byte boundary!")
    ...
    if bytesPerRow % 64 != 0 || offset % 64 != 0 { return nil }
    ...
    if let buffer = makeBuffer(bytesNoCopy: alignedBufferAddr!, length: Int(calculatedBufferLength), options: .storageModeShared, deallocator: nil) {
